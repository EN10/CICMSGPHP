...
$userSearch = mysql_query("SELECT * FROM users WHERE UID = '$_SESSION[id]'");

while($userRow = mysql_fetch_array($userSearch))
  { // match password
    if ($userRow['password'] == sha1($_POST[CPW]))
    { $npw = mysql_real_escape_string($_POST[NPW1]);
	  $sql = "UPDATE users SET password=sha1('$npw') WHERE UID='$_SESSION[id]'";
